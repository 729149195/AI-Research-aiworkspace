...
| Skill | 负责什么 | 交付什么 |
|---|---|---|
| auto-route | 每次改稿的默认入口、实际差异与上下文沉淀、自动分派 | 不覆盖科学事实的变更记录、分组待办、全局复核与跨会话交接 |
...
新增其他自定义能力：在 assets/skills 新增符合格式的文件；注册到 `skills.py` 的 SKILLS/必要 ROUTES；在 release.json 添加受管目标；更新测试中的数量和角色覆盖。若要管理 SKILL.md 以外的资源，显式扩展打包与 owned 白名单，先写路径/删除/冲突/回滚测试，禁止用通配符开放整个研究目录。

## 评估与工具发现

[行为测试场景](../evals/scenarios.json) 覆盖十个 Skills；[评估说明](../evals/README.md) 区分格式契约、确定性软件测试和真实宿主行为评估。现有121项软件测试与实际 demo/更新演练不自动证明写作或模型推理质量。

Researcher 遇到未知任务时比较可用数据库、模型、统计/绘图工具或外部 Skill 的原始来源支持、可复现性、成本、许可证及信息流向。先记录建议并获得安装/付费/联网授权；不会不加审查拉取远程脚本或自动升级整套第三方依赖。

## 0.2.0 默认入口：Auto-route

每次改稿或研究讨论由 auto-route 隐式接入，捕获观察、保留必要的上下文、合并同一 Skill 的持久待办并交接下一会话。规范研究状态保持原有提案/审批与核验边界。自动捕获记录和观察快照属于非证据元数据，不构成第二份研究事实库。旧 Schema 1 项目通过既有增量更新补入 Skill、入口指令和已注册宿主副本；Hook 要单独授权。完整操作见 [AUTO_ROUTE](AUTO_ROUTE.md)。
