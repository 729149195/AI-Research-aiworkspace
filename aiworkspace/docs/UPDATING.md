...
独立仓库使用全新的 Git 历史。请将独立仓库 clone 到新目录，为它创建新的专用虚拟环境，再对现有论文执行 `rw upgrade check` / `rw upgrade apply`。保留原论文目录及 `.rw/framework.json`，不要重新 init、修改旧 clone 的 origin 后强行合并，或删除旧框架。具体命令见 [独立仓库与已有论文](NEW_REPOSITORY.md)。
